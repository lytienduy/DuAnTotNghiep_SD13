...
const Size = () => {
  ...
  // Gọi API lấy danh sách Size từ Spring Boot
  useEffect(() => {
    setLoading(true);
    console.log("Gọi API với page =", page, "size =", rowsPerPage);
    axios
      .get(
        `http://localhost:8080/api/size/phan-trang?page=${page}&size=${rowsPerPage}`
      )
      .then((response) => {
        console.log("Dữ liệu nhận được:", response.data);
        setSizeList(response.data.content);
      ...
      <Paper sx={{ padding: 3, mb: 3 }}>
              ...
              <Grid container spacing={2} alignItems="center">
                ...
                <Grid item xs={12} md={4}>
                  ...
                  <TextField
                    label="Tìm kiếm theo tên kiểu dáng"
                    value={""}
          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell><strong>Tên Size</strong></TableCell>
            ...
            <TableBody>
              {sizeList.map((size, index) => (
                <TableRow key={size.id}>
                  ...
                  <TableCell>{size.tenSize}</TableCell>
                  <TableCell>{size.moTa || "Không có mô tả"}</TableCell>
                  <TableCell sx={{ color: size.trangThai ? "green" : "red" }}>
                    {size.trangThai ? "Hoạt động" : "Ngừng hoạt động"}
                  </TableCell>
       ...
       <Box
          ...
          >
            <Box display="flex" alignItems="center">
              ...
              <Typography ml={2}>Kích Thước</Typography>
            </Box>
            <Box display="flex" alignItems="center">
              <IconButton
