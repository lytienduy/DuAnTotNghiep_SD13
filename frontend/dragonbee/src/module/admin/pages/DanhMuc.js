...
import { Sync } from "@mui/icons-material"; // hoặc bất kỳ icon nào bạn thíchz
const DanhMuc = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `http://localhost:8080/api/danhmuc/phan-trang?page=${page}&size=${rowsPerPage}`
      )
      .then((response) => {
        console.log("Dữ liệu nhận được:", response.data);
        setDanhMucList(response.data.content);
  ...
  return (
    <Box>
            ...
            <TableBody>
              {danhMucList.map((danhMuc, index) => (
                <TableRow key={danhMuc.id}>
                  ...
                  <TableCell
                    sx={{ color: danhMuc.trangThai ? "green" : "red" }}
                  >
      ...
      >
        ...
        <Box display="flex" alignItems="center">
          <IconButton onClick={() => setPage((prev) => Math.max(prev - 1, 0))}>
            <ChevronLeft />
          </IconButton>
          {renderPageNumbers()}
          <IconButton
