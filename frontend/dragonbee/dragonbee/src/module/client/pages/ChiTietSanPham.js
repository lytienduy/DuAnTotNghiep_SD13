...
const ChiTietSanPham = () => {
  ...
  var selectedSizeReuse =
    product?.listHinhAnhAndMauSacAndSize?.[selectedColor]?.listSize?.[
      selectedSize
    ];
  var selectedColorReuse =
    product?.listHinhAnhAndMauSacAndSize?.[selectedColor];
  ...
  const getSanPhamChiTiet = async () => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:8080/spctClient/getListSanPhamChiTietTheoMau/${id}`,
        cart, // Gửi mảng JSON
        {
          headers: {
  ...
  const addVaoGioHang = () => {
    if (selectedSize === -1) {
      showErrorToast("Bạn chưa chọn size sản phẩm");
      return;
    } else {
      try {
        ...
        const index = cart.findIndex(
          (item) => item.idSPCT === selectedSizeReuse.idSPCT
        );
        if (index !== -1) {
          ...
          cart.push({
            ...
            anhSPCT:
              selectedColorReuse?.listAnh === null
                ? null
                : selectedColorReuse?.listAnh[0],
        ...
        showErrorToast(
          "Thêm vào giỏ hàng thất bại. Đã có lỗi xảy ra vui lòng thử lại"
        );
      }
    }
  };
  ...
  const handlePrevImage = () => {
    setSelectedImageIndex((prevIndex) => {
      const newIndex =
        (prevIndex - 1 + selectedColorReuse?.listAnh?.length) %
        selectedColorReuse?.listAnh?.length;
      scrollToThumbnail(newIndex);
      return newIndex;
    });
  };
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 4, marginBottom: -8 }}>
      <Breadcrumbs aria-label="breadcrumb">
        ...
        <Link
          color="inherit"
          href="http://localhost:3000/sanPham"
          sx={{ fontSize: "14px", textDecoration: "none" }}
        >
          Sản phẩm
        </Link>
        ...
        <Grid container spacing={4}>
          ...
          <Grid item xs={12} md={6}>
              ...
              {product?.listHinhAnhAndMauSacAndSize?.map((item, index) => (
                <IconButton
                  ...
                  sx={{
                    ...
                    border:
                      selectedColorReuse?.mauSac.id === item.mauSac.id
                        ? "2px solid blue"
                        : "none",
                    padding: 0,
            ...
            <Box mt={2}>
              ...
              <Box display="flex" gap={1} mt={1}>
                {selectedColorReuse?.listSize?.map((size, index) => (
                  <Button
                    variant={
                      selectedSizeReuse?.id === size?.id
                        ? "contained"
                        : "outlined"
                    }
                    onClick={() => {
                      setSelectedSize(index);
                      setSelectedIDSize(size.id);
                    }}
                    sx={{
                      minWidth: 50,
                      borderRadius: 2,
                      bgcolor:
                        selectedSizeReuse?.id === size?.id ? "black" : "white",
                      color:
                        selectedSizeReuse?.id === size?.id ? "white" : "black",
                      borderColor: "black",
                      "&:hover": {
                        bgcolor:
                          selectedSizeReuse?.id === size?.id
            ...
            <Box mt={2} display="flex" alignItems="center">
              ...
              <IconButton
                onClick={() => tangSoLuong()}
                disabled={quantity >= (selectedSizeReuse?.soLuong ?? 0)}
              >
                <AddIcon />
              </IconButton>
            </Box>
              ...
              >
                ...
                >
                  {Array.from({ length: 5 }).map((_, index) => {
                    const product =
                      productData[(start + index) % productData.length];
                    return (
