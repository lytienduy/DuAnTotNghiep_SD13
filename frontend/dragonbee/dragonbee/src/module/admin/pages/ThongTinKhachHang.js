...
const ThongTinKhachHang = () => {
  ...
  useEffect(() => {
    if (!id) return;
    axios
      .get(`http://localhost:8080/khach-hang/${id}`)
    ...
    const fetchData = async () => {
      for (let index = 0; index < khachHang.diaChiDtos.length; index++) {
        ...
        const provinceId = provinces.find(
          (province) => province.ProvinceName === item.thanhPho
        )?.ProvinceID;
        ...
        try {
          const districtResponse = await axios.get(
            ...
            {
              headers: {
                token: "2ae73454-f01a-11ef-b6c2-d21b7695c8d0",
          ...
          setKhachHang((prev) => ({
            ...
            diaChiDtos: prev.diaChiDtos.map((item, i) => {
              ...
              return {
                ...item,
                districts: districtResponse.data.data.filter(
          ...
          const districtId = districtResponse.data.data.find(
            (district) => district.DistrictName === item.huyen
          )?.DistrictID;
  ...
  const capNhatKhachHang = () => {
    axios
      .put(`http://localhost:8080/khach-hang/${id}`, khachHang)
      .then((response) => {
        toast.success("Cập nhật khách hàng thành công");
        setTimeout(() => navigate("/admin/khachhang"), 2000);
      })
  ...
  const themKhachHang = () => {
    axios
      .post("http://localhost:8080/khach-hang/", khachHang)
      .then((response) => {
  ...
  const deleteAddressHandler = () => {
    if (khachHang.diaChiDtos[selectedAddressIndex]?.id) {
      axios
        .delete(
          `http://localhost:8080/dia-chi/${khachHang.diaChiDtos[selectedAddressIndex]?.id}`
        )
  ...
  const makeDefaultAddress = (index) => {
    if (khachHang.diaChiDtos[index]?.id) {
      axios
        .put(
          `http://localhost:8080/dia-chi/${khachHang.diaChiDtos[index]?.id}/mac-dinh`
        )
        .then((response) =>
      ...
      <Box key={index} sx={{ padding: 1 }}>
        <Box sx={{ display: "flex", justifyContent: "space-between" }}>
          <Box sx={{ display: "flex", gap: 1, alignItems: "center" }}>
            ...
            {khachHang.diaChiDtos[index]?.macDinh ? (
              <IconButton>
                <Star style={{ color: "red" }} />
              </IconButton>
            ) : (
              <IconButton onClick={() => makeDefaultAddress(index)}>
                <Star />
              </IconButton>
            )}
          </Box>
        ...
        <Box sx={{ display: "flex", gap: 1 }}>
            ...
            >
              {provinces.map((province, i) => (
                <MenuItem
                  key={i}
                  onClick={() =>
                    updateDistrictsHandler(index, province.ProvinceID)
                  }
          ...
          <FormControl sx={{ flex: 1 }}>
            ...
            >
              {khachHang.diaChiDtos[index].districts?.map((district, i) => (
                <MenuItem
                  key={i}
                  onClick={() => updateWardsHandler(index, district.DistrictID)}
          ...
          <FormControl sx={{ flex: 1 }}>
            ...
            >
              {khachHang.diaChiDtos[index].wards?.map((ward, i) => (
                <MenuItem key={i} value={ward.WardName}>
                  {ward.WardName}
                </MenuItem>
              ))}
            </Select>
        ...
        <Box sx={{ display: "flex", gap: 1 }}>
          <Box sx={{ flex: 1 }}>
            <p>Số nhà </p>
            <TextField
              sx={classNames.textField}
              type="text"
              value={khachHang.diaChiDtos[index]?.soNha}
              onChange={(e) => addressChangeHandler(e, index, "soNha")}
            ></TextField>
          </Box>
          <Box sx={{ flex: 1 }}>
            <p>Đường </p>
            <TextField
              sx={classNames.textField}
              type="text"
              value={khachHang.diaChiDtos[index]?.duong}
              onChange={(e) => addressChangeHandler(e, index, "duong")}
            ></TextField>
          </Box>
        </Box>
      </Box>
  ...
  return (
    <Box>
      ...
      <Box sx={classNames.container}>
          ...
          <Box sx={{ ...classNames.customerInfo, width: "30%" }}>
            ...
            <Box>
              ...
              <TextField
                ...
                onChange={(e) => infoChangeHandler(e, "tenKhachHang")}
              ></TextField>
            </Box>
            <Box>
              <p>Email </p>
              <TextField
                ...
                onChange={(e) => infoChangeHandler(e, "email")}
              ></TextField>
            </Box>
            <Box>
              <p>Số điện thoại </p>
            ...
            <Box>
              ...
              <TextField
                ...
                onChange={(e) => infoChangeHandler(e, "ngaySinh")}
              ></TextField>
            </Box>
            <Box>
              <p>Giới tính</p>
        ...
        <Box sx={{ display: "flex", justifyContent: "center" }}>
          <Button
            onClick={id ? capNhatKhachHangHandler : themKhachHang}
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>Bạn có muốn cập nhập không?</DialogContentText>
        </DialogContent>
        <DialogActions>
