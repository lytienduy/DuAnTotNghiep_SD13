...
public interface TopProductRepository extends JpaRepository<HoaDonChiTiet, Integer> {
    ...
    @Query(value = """
        ...
        WHERE CAST(hd.ngay_tao AS DATE) = CAST(GETDATE() AS DATE)
        GROUP BY spct.mo_ta, spct.gia
    ...
    JOIN san_pham_chi_tiet spct ON hdct.id_san_pham_chi_tiet = spct.id
    LEFT JOIN anh_san_pham asp ON spct.id = asp.id_san_pham_chi_tiet
    WHERE hd.ngay_tao >= DATEADD(DAY, - (DATEPART(WEEKDAY, GETDATE()) + @@DATEFIRST - 2) % 7, CAST(GETDATE() AS DATE))
...
""", nativeQuery = true)
    ...
    @Query(value = """
        ...
        JOIN san_pham_chi_tiet spct ON hdct.id_san_pham_chi_tiet = spct.id
        LEFT JOIN anh_san_pham asp ON spct.id = asp.id_san_pham_chi_tiet
        WHERE hd.ngay_tao BETWEEN DATEFROMPARTS(YEAR(GETDATE()), MONTH(GETDATE()), 1) 
                           AND EOMONTH(GETDATE())
        GROUP BY spct.mo_ta, spct.gia
        ORDER BY tong_ban DESC
    """, nativeQuery = true)
    ...
    @Query(value = """
        ...
        JOIN san_pham_chi_tiet spct ON hdct.id_san_pham_chi_tiet = spct.id
        LEFT JOIN anh_san_pham asp ON spct.id = asp.id_san_pham_chi_tiet
        WHERE hd.ngay_tao BETWEEN DATEFROMPARTS(YEAR(GETDATE()), 1, 1) 
    ...
    @Query(value = """
        ...
        JOIN san_pham_chi_tiet spct ON hdct.id_san_pham_chi_tiet = spct.id
        LEFT JOIN anh_san_pham asp ON spct.id = asp.id_san_pham_chi_tiet
        WHERE hd.ngay_tao >= CAST(:startDate AS DATETIME)
